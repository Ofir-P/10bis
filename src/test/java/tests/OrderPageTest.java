...
public class OrderPageTest {
	...
	private static final Logger logger = LogManager.getLogger(OrderPageTest.class);
	...
	public void beforeClass() throws ParserConfigurationException, SAXException, IOException {
		...
		payment = Utilities.getDataFromXML("info.xml", "payment", 0);
	...
	/*  Prerequisite: Logged in https://www.10bis.co.il/ and already on Restaurant page
	 * 		Given: Client click on specific meal
	 * 		When: Adding three meals to shopping cart
	 *  	Then: He will be able to get to the payment page
	 */
	@Test(priority = 3, enabled = true, description = "Adding 3 meals to the shopping cart and going to payment page")
	public void OrderFood() throws InterruptedException, IOException, ParserConfigurationException, SAXException {

		logger.info("Selecting meals");
		main.order();
		logger.info("Selected meals successfully, heading to Payment page now");
		Assert.assertTrue(order.orderPage(payment), "Could not find the payment page, Check your logs");
		logger.info("Successfully Got into the payment page!");
